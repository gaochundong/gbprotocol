...
public class JT808_Message_Content_0x0200 extends JT808MessageContent {
    ...
    public void serialize(IVersionedSpecificationContext ctx, IJT808MessageBufferWriter writer) {
        writer.writeDWord(this.getBasicInformation().getWarningType().getValue());
        writer.writeDWord(this.getBasicInformation().getState().getValue());
        writer.writeDWord(this.getBasicInformation().getLatitude());
        writer.writeDWord(this.getBasicInformation().getLongitude());
        writer.writeWord(this.getBasicInformation().getAltitude());
        writer.writeWord(this.getBasicInformation().getSpeed());
        writer.writeWord(this.getBasicInformation().getDirection());
        writer.writeBCD(TIMESTAMP_FORMATTER.format(this.getBasicInformation().getTimestamp()));
    ...
    public void deserialize(IVersionedSpecificationContext ctx, IJT808MessageBufferReader reader) {
        this.setBasicInformation(
                BasicInformation.builder()
                        .warningType(JT808WarningType.cast(reader.readDWord()))
                        .state(JT808VehicleState.cast(reader.readDWord()))
                        .latitude(reader.readDWord())
                        .longitude(reader.readDWord())
                        .altitude(reader.readWord())
                        .speed(reader.readWord())
                        .direction(reader.readWord())
                        .timestamp(LocalDateTime.parse(reader.readBCD(6), TIMESTAMP_FORMATTER))
                        .build()
        );
